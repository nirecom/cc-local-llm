...
### CONFIG: Sleep-freeze fix and opus timeout extended to every Mac backend (2026-09-01)
Background: Opus tier (qwen3.8-flash-next-3bit-mtp) hit repeated client-side 'Connection refused' from the code-ccgw path only (native Claude Code was unaffected all day). litellm.log/pmset -g log/pmset -c sleep=1 correlated to the second: the Mac's 1-minute AC idle-sleep timer plus Wake-on-LAN produced a DarkWake state during inference that could not finish the request. The 2026-07-07 sleep-freeze fix (caffeinate -ism) had only ever been applied to ds4-server -- every other llama-swap entry on this Mac, including today's opus backend, was unprotected. Separately, litellm-server/config.yaml's fable and opus routes have never carried an explicit timeout (unlike haiku/sonnet), so a long opus prefill (206,819 tokens measured at 717s on qwen3.8-flash-next-3bit-mtp) could hit LiteLLM's shorter default and fail even once the sleep freeze is fixed.
Changes: llama-swap/m5-max-128gb/config.yaml: wrapped all 11 model entries' cmd in caffeinate -ism (previously only deepseek-v4-flash via its sh -c wrapper); model-annotations.yaml header documents the class-wide policy. litellm-server/config.yaml: added timeout: 3600 to the opus route, matching the MLX_VLM_TOKEN_QUEUE_TIMEOUT ceiling already set on the qwen3.8-flash-next-3bit-mtp llama-swap entry, so LiteLLM never cuts the connection before the backend itself gives up. pmset -c sleep 0 / the System Settings sleep-prevention toggle was considered again but not reapplied -- docs/history.md already recorded it as unreliable for this exact failure in 2026-07.

### FEATURE: PR #90 — feature/89-litellm-env-litellm-server-config (2026-09-02, 7e482a7)
Background: Routing-key management for LiteLLM (LITELLM_HAIKU_MODEL, LITELLM_SONNET_MODEL, LITELLM_FABLE_MODEL, LITELLM_OPUS_MODEL, CCGW_SUBAGENT_MODEL) lived in per-machine .env files -- not git-tracked, and kept in sync by hand across hosts.
Changes: Moved routing-key ownership into git-tracked litellm-server/config.yaml via new ccgw_tiers: annotations per model entry; scripts/set-model.sh and the ccgw launchers (code-ccgw.sh/.ps1) now read tier->model mappings directly from config.yaml instead of environment variables. Added CCGW_AUTO_PULL to refresh config.yaml from the server before each launch. The five per-host routing-key env vars are retired: unset is silent, but a stale .env still carrying one prints a migration warning naming the key and pointing at config.yaml. (#89)

### CONFIG: opus tier re-quantized as a mixed 3/8-bit MLX build (2026-09-03)
Background: The opus tier ran on a self-converted uniform 3bit/g32 MLX build of Qwen3.8-Flash-Next, which degenerated into token repetition from the first turn of a session. MTP, APC, long context and the architecture itself were each ruled out by substitution; unsloth's UD-IQ3_XXS GGUF of the same weights, at a lower effective bit rate (3.705 bpw against 4.03 for bits + 32/group_size on the broken build), behaved correctly. That localized the fault to the allocation rather than the bit budget: a uniform pass gives the routers, the QSA indexer and the PLE n-gram table the same 3 bits as the routed experts, and those three make discrete selections that a 3-bit grid inverts.
Changes: Converted a mixed-precision build -- routers and QSA indexer left unquantized, PLE n-gram table at 4bit/g32, routed experts at 3bit/g64, everything else 8bit/g64 -- measuring 4.094 bits per weight and 85 GB, 1.8 GB above the build it replaces. The size ceiling is not aesthetic: the memory line measured in tuning.md means a gigabyte of weights costs roughly 6k tokens of reachable context with APC on, and the tier must hold the 76,800-token effective auto-compact window inside a 112 GB budget. Adopting pipenetwork/Qwen3.8-Flash-Next-MLX-mixed-4_8bit verbatim (98.9 GiB) would have cut the tier to ~16k tokens, and adding 4-bit expert down projections (89.5 GiB) leaves no margin at all -- hence 3 bits on the routed experts only. Rationale and the full build/context table are recorded in tuning.md under 'What the memory line costs a re-quantization'. The build is produced and audited; switching the llama-swap entry and the opus route over to it is tracked in #91.

### CONFIG: DeepSeek-V4.1-Flash added as a separate switchable llama-swap entry; deepseek-v4-flash's model path pinned (2026-09-13)
Background: Downloaded DeepSeek-V4.1-Flash-Q2.gguf (365.7GB) into ~/git/ds4/gguf/ via download_model.sh, wanting to try it. ds4's download_model.sh repoints the repo's ds4flash.gguf symlink to whichever model it last selected, and the existing deepseek-v4-flash llama-swap entry (routed to the LiteLLM fable tier) had no explicit --model flag, so it depended on that symlink's default. The download therefore silently repointed the production fable-tier entry's real weights from V4 Flash (97.6GB) to V4.1-Flash (365.7GB).
Changes: llama-swap/m5-max-128gb/config.yaml: pinned deepseek-v4-flash's --model to its gguf path explicitly, restoring it to V4 Flash regardless of the symlink's current target. Added a new deepseek-v4.1-flash entry, also with an explicit --model pin, using --ssd-streaming (365.7GB does not fit the 128GB unified memory ceiling resident) and first-pass conservative flags (ctx 32768, automatic expert-cache budget, separate --kv-disk-dir from deepseek-v4-flash) per ds4's own docs/SSD_STREAMING.md guidance -- pending real measurement. Not routed to any LiteLLM tier yet. model-annotations.yaml and docs/tuning.md updated to match.

### CONFIG: llama.cpp UD-Q3_K_XL evaluated for opus tier adherence improvement (issue #92) (2026-09-22)
Background: The opus tier (qwen3.8-flash-next-mixed-3_8bit, MLX) showed insufficient prompt adherence in agents workflow sessions. Issue #92 proposed trying unsloth's UD-Q3_K_XL GGUF of the same checkpoint run under llama.cpp (llama-server 0.4.1, qwen4exp arch, PR #27742 merged to master 2026-08-27). Two questions: (1) does imatrix calibration improve adherence, and (2) can 100k context still be served.
Changes: llama-server 0.4.1 confirmed via Homebrew. Downloaded 84 GB UD-Q3_K_XL GGUF (3-shard split). 100k gate: 98,939-token prompt at --ctx-size 106496 --parallel 1 completed with 0 swap pages, prefill 350 tok/s cold single-shot at 100k, decode 14.5 tok/s cold. A later warm same-probe A/B against the production MLX mixed-3_8bit (both warm, this M5 Max) is the valid throughput comparison, not the retired uniform-3bit build: prefill is at parity at ~13k (MLX 855.9 vs UD 870.1) but MLX pulls ahead as context grows (53k 950.6 vs 551.7 = 1.7x, 100k 678.7 vs 358.8 = 1.9x); decode runs the other way, UD leading at short/mid context (13k 24.1 vs 37.4, 53k 21.7 vs 26.9) and converging by 100k (18.8 vs 17.2, decode measured over 16-19 tokens so directional). Against the reference zenn article (M4 Max, UD tops out at 53k: 143 prefill / 12.2 decode) our warm UD is 2.5-3.9x faster, explained by --flash-attn on + warm mmap + M5-vs-M4 bandwidth with a matching context-scaling shape (no measurement anomaly). Critical memory finding: llama.cpp mmaps weights as clean reclaimable file-cache (footprint phys_footprint 3.8 GB committed, 57 GB mapped-file clean); the 112 GB Metal working-set budget that MLX obeys does not bind here. 100k NFR met with more headroom than the MLX build. Adherence: easy battery UD 8/8 vs MLX 7/8 (one count probe); hard agent-realistic battery (12 simultaneous constraints, buried negative, diff-only, strict JSON schema, instruction-over-prior, exact stop, YAML persona lock, 16k-token context) UD 6.83/8 vs MLX 6.92/8 -- effectively a tie. The imatrix-calibration hypothesis for adherence improvement is not supported. UD is not adopted as the production opus tier: adherence gain not confirmed, and while UD decode is competitive its prefill scales worse than MLX at the 100k context this tier must serve. The mmap memory model and measured throughput are recorded in tuning.md as reference for future runtime selection.
