...
_set_env_var() {
    _sev_key="$1"
    _sev_val="$2"
    _sev_tmp="$(_ccgw_begin_write "$DOTENV_FILE")"
    ...
    _ccgw_commit_write "$_sev_tmp" "$DOTENV_FILE"
}
...
_set_backend_model() {
    ...
    _sbm_tmp="$(_ccgw_begin_write "$LITELLM_CONFIG")"
    ...
    _ccgw_commit_write "$_sbm_tmp" "$LITELLM_CONFIG"
}
