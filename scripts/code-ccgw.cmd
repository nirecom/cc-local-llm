...
rem Each var used below is extracted individually via the shared
...
rem them -- same fix pattern as scripts/litellm-start.cmd. This script does not call
rem docker compose, so --env-file is not applicable here.
set "DS4_ENV_FILE=%~dp0..\.env"
if exist "%DS4_ENV_FILE%" (
    for %%V in (LITELLM_ANTHROPIC_BASE_URL CCGW_ANTHROPIC_BASE_URL DS4_ANTHROPIC_BASE_URL LITELLM_VIRTUAL_KEY CCGW_API_KEY DS4_API_KEY CCGW_CA_CERT DS4_CA_CERT LITELLM_OPUS_MODEL LITELLM_SONNET_MODEL LITELLM_HAIKU_MODEL) do call "%~dp0lib\load-env-var.cmd" "%DS4_ENV_FILE%" %%V || exit /b 1
)
